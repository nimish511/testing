...
def load_data(image_list, mask_list):
    ...
    # uniq_val = tf.unique(mask)
    mask = tf.where(mask >0, 1, 0)
    # mask = tf.where(mask == uniq_val[1], 1, 0)
...
#def load_data(image_list, mask_list):
#    image = read_image(image_list)
#    input_mask = read_image(mask_list, mask=True)

    #uniq_val = np.unique(mask)
#    input_mask = tf.where(input_mask == 1, 1, 0)
#    input_mask = tf.where(input_mask == 2, 2, 0)
    #mask = tf.where(mask == uniq_val[1], 1, 0)
    #mask = tf.where(mask == uniq_val[2], 2, 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-wd', '--weight-decay', default=5e-5, type=float, help='Weight Decay')
    ...
    # To load new model
    #model = tf.keras.saving.load_model('final_model.h5')
    ...
    callbacks = [
        ...
        ReduceLROnPlateau(factor=0.1, patience=10, min_lr=LEARNING_RATE, verbose=1),
